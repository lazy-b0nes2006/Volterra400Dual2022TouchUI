...
class filamentSensor:
    ...
    def filamentSensorHandler(self, data):
        ...
        if triggered_extruder0 and self.MainUIObj.stackedWidget.currentWidget() not in [self.MainUIObj.changeFilamentPage, self.MainUIObj.changeFilamentProgressPage,
        ...
        if triggered_extruder1 and self.MainUIObj.stackedWidget.currentWidget() not in [self.MainUIObj.changeFilamentPage, self.MainUIObj.changeFilamentProgressPage,
                                  self.MainUIObj.changeFilamentExtrudePage, self.MainUIObj.changeFilamentRetractPage]:
            if dialog.WarningOk(self.MainUIObj, "Filament outage in Extruder 1"):
                pass

        if triggered_door:
            if self.printerStatusText == "Printing":
                no_pause_pages = [self.MainUIObj.controlPage, self.MainUIObj.changeFilamentPage, self.MainUIObj.changeFilamentProgressPage,
                                  self.MainUIObj.changeFilamentExtrudePage, self.MainUIObj.changeFilamentRetractPage]
